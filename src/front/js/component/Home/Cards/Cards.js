...
export default function Cards({ data }) {
  ...
  const { id, title, img_url, price, ratings, start_day } = data;
  return (
    <div className="card box mb-2 me-3 bg-body rounded">
      <Link to={`/event/${id}`}>
        <img
          src={img_url ? img_url : imagen}
      ...
      <PlaygroundSpeedDial
          to={`https://api.whatsapp.com/send?text=Te%20invito%20a%20este%20evento%20https://3000-leonarmed-wowapp-5gmb4uhbzop.ws-us75.gitpod.io/event/${id}`}
        />
    </div>
  );
}
